...
import { Check, X, ChevronDown, Settings2, Trash, ArrowDown, Sparkles, Layout, Zap, Pencil, FileText } from 'lucide-react';
        ...
        <div className="flex-1 min-w-0 mr-4">
          {isEditingTitle ? (
            <div className="flex items-center gap-2 max-w-md">
              <Input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (renameSession(activeSessionId, editTitle), setIsEditingTitle(false))} autoFocus className="h-9 py-1 rounded-xl" />
              <Button size="icon" variant="ghost" className="h-9 w-9 text-turquoise" onClick={() => (renameSession(activeSessionId, editTitle), setIsEditingTitle(false))}><Check className="w-5 h-5" /></Button>
            </div>
          ) : (
        ...
        <div className="flex items-center gap-2">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 rounded-2xl">
              ...
              <DropdownMenuItem onClick={() => exportToText(messages, activeSession?.title || 'Chat')} className="gap-2 rounded-xl cursor-pointer"><FileText className="w-4 h-4" /> Export MD</DropdownMenuItem>
              <DropdownMenuItem onClick={() => clearCurrentSession(activeSessionId)} className="gap-2 rounded-xl cursor-pointer text-destructive"><Trash className="w-4 h-4" /> Clear History</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
        ...
        <div className="space-y-8 max-w-4xl mx-auto pb-12">
          {messages.length === 0 && !isProcessing ? (
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-6">
              <div className="w-20 h-20 rounded-full bg-accent flex items-center justify-center animate-float"><Sparkles className="w-10 h-10 text-turquoise" /></div>
      ...
      <AnimatePresence>
        {showScrollBottom && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute bottom-32 left-1/2 -translate-x-1/2 z-30">
            <Button size="icon" className="rounded-full bg-background border border-border shadow-lg" onClick={() => scrollRef.current?.scrollIntoView({ behavior: 'smooth' })}><ArrowDown className="w-5 h-5" /></Button>
          </motion.div>
        )}
      </AnimatePresence>
